...
xsim-run:
	$(FUSESOC) --cores-root . run --no-export --target=sim --tool=xsim $(FUSESOC_FLAGS) --run openhwgroup.org:systems:core-v-mini-mcu $(FUSESOC_PARAM)  2>&1 | tee sim.log
	
## Opens Vivado gui in view only mode to view the waveform generated by the last xsim simulation
xsim-waves:
	cd $(XSIM_DIR) && \
	xsim -gui ./waveform.wdb &
